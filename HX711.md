# HX711 driver rewrite — working log

> Single source of truth for the HX711 load-cell driver rewrite, fix
> iterations, and test campaigns on the CC1 test printer (carbon2u).
> **Convention:** append new test results and iteration notes under
> `## Updates` at the bottom — do not create new HX711*.md/.log files.
>
> Absorbed 2026-07-23: `HX711-REWRITE.md` (cosmos-merge),
> `HX711-bed-mesh-test-20260723.md` and `hx711-mesh-loop-20260723-iter{1..5}.log`
> (cosmos-nightly). Sections 1-4 below are their content, unedited.

## Current state (updated 2026-07-23)

- kalico `paul/hx711-reliability` @ `c368f84e`: **10/10 consecutive
  BED_MESH_CALIBRATE @ 60C**, zero hard faults in 392k frames.
- Cosmos PR #265 (`doom/loadcell`) pins that stack (r10) + cleaning-pass
  tolerance fix; PR artifact (run 30029412594) flashed on carbon2u.
- In flight: 20x mesh hot-soak at 100C (results appended below when done).
- Idea pool: read-on-DRDY sampling (sample only when each ADC signals
  ready) to make invalid frames structurally impossible; minimal-MCU
  direction (MCU streams raw+faults, Klippy owns smarts).

---

## Rewrite plan 2026-07-25: read-on-DRDY driver

Active plan (supersedes the idea-pool note below; full detail:
`.hermes/plans/2026-07-25_174500-hx711-read-on-drdy-rewrite.md`).
Branch: `paul/hx711s-new` (james's `hx711s-new` @ 413e9e38 + the two
load_cell_probe race fixes). Validated stack stays on
`paul/hx711-reliability` @ c368f84e as the shipping baseline.

**Why a new architecture:** both current drivers detect the torn-read race
(~0.1-0.4% of frames: ~50us read window vs 11.6ms conversion period) and
then classify the wreckage. The datasheet and every reference impl (Prusa
HX717, Linux IIO, upstream Klipper, Elegoo blob) wait for DRDY before
reading. IRQ-per-chip is blocked by hardware (PC8 and PA8 share EXTI8), so
the fix is verify-and-retry, not interrupts.

**Design:** per-chip WAIT_READY -> fast read -> verify post-read DOUT high
-> emit timestamped sample. Torn read: retry on next DRDY (bounded x2),
then marked gap. 4-channel sum for the MCU trigger held until all channels
verify (~1.5-period deadline, else STUCK fault on the laggard). Desync
latches -> bounded power-cycle -> settle -> requalify.

**Deletes:** timing-miss skip taxonomy, streak escalation, most quality
bits, most of the recovery FSM, generic probe.py retry (9c1936ef).
**Keeps:** spike filter (glitches are protocol-valid), bulk-overflow
tolerance, per-frame timestamps, the two load_cell_probe race fixes.

**Validation bar:** 10/10 mesh @60C and 20/20 mesh @100C with zero hard
faults and zero recoveries, matching c368f84e, before any PR switch.

---

## 1. Rewrite analysis (was cosmos-merge/HX711.md)
...
## 2. Design and iteration narrative (was cosmos-merge/HX711-REWRITE.md)
...
## Updates 2026-07-26: read-on-DRDY driver rewrite (branch paul/hx711s-new)

Fresh start from james's `hx711s-new` (`413e9e38`) per the 2026-07-25 rewrite
plan (above). Constraint: reliability in the MCU driver only, no changes to
other working kalico code. Flash loop: cosmos-nightly `rc.build.cc1.emmc`,
scp swu, `flash`, reboot.

### Iteration 1 (`9efa8c34`): in-driver recovery
- Torn reads (~0.4% by physics: 50us read window / 11.6ms period) are no
  longer consumed or discarded: emit nothing, re-read on next DRDY, bounded
  2 retries (then degrade to host hold-last-value). Cites: Prusa HX717,
  Linux IIO hx711, upstream Klipper hx71x, Elegoo bounded ready-poll.
- Desync (gain-pulse mismatch): in-driver power cycle (datasheet: SCK high
  >60us), 4 settling conversions discarded, ~115ms pause, RECOVERED marker
  frame carries retry/recovery tallies. Host logs, no forced restart
  (previously a forced restart killed active probe sessions).
- Stuck ADC (no DRDY ~2.5 periods): same power-cycle path.

### Iteration 1b (`b9a658cb`): impulse hold-confirm (phantom triggers)
- Recorded tap curves showed the killer class: single-sample force impulses
  (~220g) at Z-move starts (accel kicks ringing the bed), instantly
  reverting, firing first-crossing phantom triggers up to 1.2mm above real
  contact -> flat tap window -> TAP_CHRONOLOGY.
- Fix: sum-level hold-confirm in the driver's trigger feed (raw stream
  untouched). Threshold 190g, hold one sample, release only if confirmed
  sustained.

### Iteration 2 (`dd612373`): threshold under trigger_force + sign-aware confirm
- Round-2 campaign died at attempt 5 on "Probe triggered prior to movement".
  Raw force capture (182k samples, load_cell/dump_force mux -> JSONL) caught
  it: isolated +87g single-sample blip 0.5s after travel settled. 87g < 190g
  hold threshold, > 75g trigger force -> phantom arming.
- Threshold now 67g (under trigger_force). Confirm now sign-aware (up-jump
  confirms if next sample >= pending - threshold): the old symmetric check
  could never confirm a monotonic ramp.
- Validated offline by replaying the 182k-sample capture through a model of
  the filter: 72/72 phantom crossings removed, 433/433 real ramps trigger.

### Campaign results (carbon2u)
- Round 1 (b9a658cb): FAIL attempt 1, pre-movement trigger (354s).
- Round 2 (b9a658cb + capture): FAIL attempt 5, same signature -> captured.
- Round 3 (dd612373): **10/10 PASS** at 60C (~417s each). 0 in-driver
  recoveries, 50 glitch-filter drops, 1 retry-absorbed TAP_SHAPE_INVALID
  (benign class the macro RETRY policy exists for).
- Hot soak (dd612373): **20/20 PASS at 100C** (~2.5h, chamber ~60C).
  0 in-driver recoveries, 0 bad taps, 10 glitch-filter drops.
  Combined record for the rewrite: 30/30 macro runs, 0 faults, 0 recoveries
  — matching the previous stack's validation with a driver-only diff
  (3 commits: 9efa8c34, b9a658cb, dd612373).

### Instrumentation
- `/tmp/force_dump.py` (host): subscribes klippy `load_cell/dump_force` mux
  over /run/klippy.sock, writes JSONL. NOTE: /tmp on carbon2u is tmpfs —
  redeploy after every flash/reboot. Sensor name for the mux is
  `load_cell_probe`.
- `TAP_RECORDER_START FILE=...` records tap JSONL (needs `[tap_recorder]`
  in printer.cfg) — captures valid + invalid taps, incl. move queue.
